...
import createSigmaLasso from "../plugins/sigma/lasso";
import { nodeRenderer, edgeRenderer } from "../plugins/sigma/renderers";

import { downloadDataUrl } from "../utils/downloads";
import { exportPNG, exportSVG } from "../utils/networks";

import ZoomControls from "./ZoomControls.react";
import NetworkControls from "../containers/NetworkControls.react";
...
class Network extends React.PureComponent {
  ...
  downloadPNG = () => {
    const dataUrl = exportPNG(this.sigmaInst);
    downloadDataUrl(
      dataUrl,
      "network.png",
      "image/png",
  ...
  downloadSVG = () => {
    const dataUrl = exportSVG(this.sigmaInst);
    downloadDataUrl(
      dataUrl,
      "network.svg",
      "image/svg+xml",
    ...
    return (
      ...
      >
        ...
        <NetworkControls
          ...
          onShuffleNodes={() => this.startLayout()}
        />
        <ZoomControls
          className={classnames({ visible: this.props.controls })}
          onReset={() => this.handleZoomReset()}
          onZoomIn={() => this.handleZoomChange(+1)}
          onZoomOut={() => this.handleZoomChange(-1)}
        />
      </div>
    );
  }
