...
export default class extends React.Component {
  ...
  static propTypes = {
    ...
    allMarkerData: PropTypes.arrayOf(
      PropTypes.shape({
        position: MapPosition.isRequired,
        rows: PropTypes.array.isRequired,
      })
    ).isRequired,
    activeMarkerData: PropTypes.arrayOf(
      PropTypes.shape({
        position: MapPosition.isRequired,
        rows: PropTypes.array.isRequired,
  ...
  componentDidMount() {
    ...
    addExportCallback('map-png', () => exportHtmlElement(this.leafletMapRef));
  }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.props.activeMarkerData !== prevProps.activeMarkerData ||
  ...
  createLeafletMap() {
    const { type = 'mapbox', style } = this.props;
  ...
  drawMarkers() {
    this.markerLayer.setOptions({
      markerData: this.props.activeMarkerData,
  ...
  refitMapBounds(point) {
    ...
    } else if (this.props.activeMarkerData.length !== 0) {
      if (type === 'mapbox') {
        this.map.fitBounds(this.props.activeMarkerData.map(marker => marker.position));
      } else if (type === 'imageoverlay') {
        this.map.fitBounds(bounds);
      }
    }
  ...
  renderInfoWindow = () => {
    if (this.state.infoMarkerPosition !== null) {
      const infoMarker = this.props.activeMarkerData.find(x => x.position === this.state.infoMarkerPosition);
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        />
        <div
          className="mr-leaflet" id="leaflet-map"
          ref={c => { this.leafletMapRef = c; }}
          style={{ width, height }}
        >
        </div>
        <MapControls />
